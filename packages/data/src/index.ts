...
import { SurfaceSnapshot, EventWindow, RiskState, OptionContract } from '../../core/src/types';
 ...
 * defaults.
 */
function validateSurfaceSnapshot(raw: any): { snapshot: SurfaceSnapshot | null; warnings: string[] } {
  ...
  const requiredFields: Array<keyof SurfaceSnapshot> = [
    'instrument',
    'timestamp',
    'spotPrice',
    'realisedVol',
  ...
  // Validate contracts
  const contracts: OptionContract[] = Array.isArray(raw.contracts) ? raw.contracts : [];
  contracts.forEach((c: any, idx: number) => {
    ['strike', 'expiry', 'type', 'openInterest', 'volume'].forEach((f) => {
      if (c[f] === undefined) warnings.push(`contract[${idx}] missing ${f}`);
    });
  });
  const snapshot: SurfaceSnapshot = {
    ...
    state: (raw.state as RiskState) || 'UNKNOWN',
  };
 ...
 */
function validateEventsArray(raw: any): { events: EventWindow[]; warnings: string[] } {
  const warnings: string[] = [];
  ...
  raw.forEach((e: any, idx: number) => {
    const missing: string[] = [];
    if (!e.instrument) missing.push('instrument');
    if (!e.eventType) missing.push('eventType');
    if (!e.description) missing.push('description');
    if (!e.start) missing.push('start');
    if (!e.end) missing.push('end');
    if (missing.length) warnings.push(`event[${idx}] missing ${missing.join(',')}`);
    events.push({
 ...
 */
export function loadSurfaceSnapshot(filePath: string): LoadResult<SurfaceSnapshot> {
 ...
 */
export function loadEventWindows(filePath: string): LoadResult<EventWindow[]> {
  const raw = readJson<any>(filePath);
  if (!raw) {
