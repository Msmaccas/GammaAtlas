...
exports.surfaceAnalystAgent = surfaceAnalystAgent;
exports.eventAnalystAgent = eventAnalystAgent;
exports.liquiditySkepticAgent = liquiditySkepticAgent;
exports.spotContextAnalystAgent = spotContextAnalystAgent;
exports.synthesisLeadAgent = synthesisLeadAgent;
const logic_1 = require("../../core/src/logic");
/**
 ...
 */
function surfaceAnalystAgent(snapshot) {
    const clusters = (0, logic_1.identifyClusters)(snapshot);
    const { risks, evidence } = (0, logic_1.computeRisks)(snapshot, [], clusters);
 ...
 */
function eventAnalystAgent(snapshot, events) {
    const clusters = (0, logic_1.identifyClusters)(snapshot);
    const { risks, evidence } = (0, logic_1.computeRisks)(snapshot, events, clusters);
 ...
 * liquidity hazards.
 */
function liquiditySkepticAgent(snapshot) {
    const clusters = (0, logic_1.identifyClusters)(snapshot);
    const { risks, evidence } = (0, logic_1.computeRisks)(snapshot, [], clusters);
 ...
 */
function spotContextAnalystAgent(snapshot) {
    const evidence = [];
 ...
 */
function synthesisLeadAgent(instrument, agentsResults) {
    const allRisks = [];
    const allEvidence = [];
